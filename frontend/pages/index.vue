      ...
      <section class="py-16 px-4 bg-brand-white">
        <div class="container mx-auto max-w-4xl">
          ...
          <div class="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div
              class="h-72 md:h-[300px] lg:h-[370px] rounded-lg flex items-center justify-center border-2 border-brand-primary bg-brand-white">
              <img src="~/assets/features.png" alt="Company Features" class="max-h-full max-w-full object-contain p-2">
            </div>
          </div>
        </div>
      </section>
        ...
        <div class="container mx-auto max-w-6xl">
          ...
          <div v-else-if="articles.length > 0" class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
            <template v-for="article in articles.slice(0, 3)" :key="article?.id || 'loading'">
              <article v-if="article && article.id"
                ...
                @mouseenter="startImageSlideshow(article.id)" @mouseleave="stopImageSlideshow(article.id)">
                <!-- Article Image -->
                <div v-if="getArticleImages(article).length > 0" class="relative h-48 overflow-hidden">
                  <div v-for="(image, index) in getArticleImages(article)" :key="index"
                    ...
                    <img :src="getImageUrl(image)" :alt="`${article.title} - Image ${index + 1}`"
                      class="w-full h-full object-cover">
                  </div>
                  <!-- Image indicator dots (for multiple images) -->
                  <div v-if="getArticleImages(article).length > 1"
                    class="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-1">
                    <div v-for="(image, index) in getArticleImages(article)" :key="index"
                      class="w-2 h-2 rounded-full transition-all duration-300"
                      :class="(currentImageIndex[article.id] || 0) === index ? 'bg-white' : 'bg-white/50'"></div>
                  </div>
                </div>
        ...
        <div class="container mx-auto max-w-6xl">
          ...
          <div v-else-if="trustedCompanies.length > 0" class="relative overflow-hidden">
            ...
            <div class="flex animate-scroll-infinite space-x-12 items-center">
              ...
              <div v-for="company in trustedCompanies" :key="company.id"
                ...
                <a v-if="company.website" :href="company.website" target="_blank"
                  class="block w-full h-full flex items-center justify-center" :title="company.name">
                ...
                <div v-else class="block w-full h-full flex items-center justify-center" :title="company.name">
                  <img :src="getCompanyLogoUrl(company.logo_url)" :alt="company.name"
                    class="max-h-full max-w-full object-contain transition-transform duration-300 group-hover:scale-110">
                </div>
              </div>
      ...
      <section class="py-16 px-4 bg-brand-dark text-brand-white">
        <div class="container mx-auto max-w-4xl text-center">
          ...
          <div class="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a href="mailto:hello@kurohaeksagon.com"
          ...
          <div class="mt-8 flex justify-center space-x-6">
            <a href="https://www.instagram.com/kurohaeksagon" target="_blank"
              class="text-brand-gray hover:text-brand-primary transition">
            ...
            <a href="https://www.linkedin.com/company/kuro-haeksagon/" target="_blank"
              class="text-brand-gray hover:text-brand-primary transition">
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path
            ...
            <a href="https://www.facebook.com/share/1CSXkK32NH/?mibextid=wwXIfr" target="_blank"
              class="text-brand-gray hover:text-brand-primary transition">
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path
